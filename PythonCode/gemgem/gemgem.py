...
# By Al Swegart al@inventwithpython.com
...
with the following keys:
  'x' and 'y' - The location of the gem on the board. 0,0 is the top left.
                There is alse a ROWABOVEBOARD row that 'y' can be set to,
...
# NUMGEMIMAGES is the number of gem type. You will need .png image
...
# is used several times, so calculate it once here and store is variables.
    ...
    while True: # main game loop
        ...
        if gameIsOver:
            if clickContinueTextSurf == None:
                ...
                clickContinueTextSurf = BIGFONT.render('Final Score: %s (Click to continue)' % (score), 1, GAMEOVERCOLOR, GAMEOVERBGCOLOR)
...
def getSwappingGems(board, firstXY, secondXY):
    ...
    # Othrewise, (None, None) is returned.
...
def canMakeMove(board):
    ...
    # move can made on it. Otherwise return False.
    ...
    # away from forming a triple, hence onOffPattern has 8 patterns.
...
def getDropSlots(board):
    # Creates a "drop slot" for each column and files the slot with a
...
def getDroppingGems(board):
    # Find all the gems that that have an empty space below them
    boardCopy = copy.deepcopy(board)
    droppingGems = []
    for x in range(BOARDWIDTH):
...
def moveGems(board, movingGems):
    ...
    for gem in movingGems:
        if gem['y'] != ROWABOVEBOARD:
            ...
            # gem is locatd above the board (where new gems come from)
...
def fillBoardAndAnimate(board, points, score):
    ...
    while dropSlots != [[]] * BOARDWIDTH:
        # do the dropping animation as long as there are move gems to drop
