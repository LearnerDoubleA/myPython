...
# Create Commons BY-NC-SA 3.0 US

import random, sys, copy, os, pygame
from pygame.locals import *

FPS = 30 # frames per seconds to update the screen.
...
def runLevel(levels, levelNum):
    ...
    while True: # main game loop
        ...
        # Draw mapSurf to the DISPLAYUSRF Surface object.
...
def decorateMap(mapObj, startxy):
    ...
    # Copy the map object so we don't modifiy the original passed
...
def startScreen():
    """Display the start screen (which has the title and instructuins)
    until the player presses a key. Returns None."""

    # Position the title image.
    titleRect = IMAGESDICT['title'].get_rect()
    topCoord = 50 # topCoord tracks where to position the top of text
...
def readLevelsFile(filename):
    ...
    for lineNum in range(len(content)):
        ...
        if line != '':
            ...
            assert len(stars) >= len(goals), 'Level % (around line %s) in %s is impossible to solve. It has %s goals but only %s stars.' % (levelNum + 1, lineNum, filename, len(goals), len(stars))
